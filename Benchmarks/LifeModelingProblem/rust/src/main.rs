...
// Via JeannotJeannot
// https://github.com/JuliaActuary/Learn/pull/7 
pub fn npv2(mortality_rates: &Vec<f64>, lapse_rates: &Vec<f64>, interest_rate: f64, sum_assured: f64, premium: f64, init_pols: f64, term: Option<usize>) -> f64 {
...
// Via JeannotJeannot
// Try to replicate npv9 of Julia benchmark
pub fn npv3(mortality_rates: &Vec<f64>, lapse_rates: &Vec<f64>, interest_rate: f64, sum_assured: f64, premium: f64, init_pols: f64, _term: Option<usize>) -> f64 {
    ...
    for (q, w) in mortality_rates.iter().zip(lapse_rates) {
        result += inforce * (premium - sum_assured * q) * discount_factor;
	discount_factor *= v;
        inforce *= 1. - (q + w);
    }
...
let r: f64 = 0.02;
    b.iter(|| {
        // use `test::black_box` to prevent compiler optimizations from disregarding
        // unused values
        test::black_box(npv2(&q,&w,r,s,p,1.0,Some(10)));
    });
}
...
let r: f64 = 0.02;
    b.iter(|| {
        // use `test::black_box` to prevent compiler optimizations from disregarding
        // unused values
        test::black_box(npv3(&q,&w,r,s,p,1.0,Some(10)));
    });
}
