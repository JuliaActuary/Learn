...
function v(v_prior,params,Zₜ) 
	...
	ṽ = min(v_p, (1-ϕ)*v_prior+ϕ * log(τ) ) + σ_v * Zₜ[2]
...
function scenario(params)
	(;σ_v,σ_0, ρ,A,B,C,months) = params
	Z = MvNormal(
		[0.,0.], #return and volatility
		[1. ρ; ρ σ_v]
	)
	draw_Z() = rand(Z)
...
md"The summary statistics expected:
...
let
	...
	scens = [scenario(params) for _ in 1:1000]
	
	μ = mean(mean(x) for x in scens)
	σ = mean(std(x) for x in scens)
	(;μ,σ)
end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "5a1d71d913998d790bc5c3cfc03d2d4b0d590e8d"
