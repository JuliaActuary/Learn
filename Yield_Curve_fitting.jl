...
# ╔═╡ c0b65e7f-ab23-43e8-820c-f62417013785
ns = Yields.Zero(NelsonSiegel(),rates,mats);

# ╔═╡ c3a86b0d-b677-47dc-8d13-0e1933b99aaa
nss = Yields.Zero(NelsonSiegelSvensson(),rates,mats);

# ╔═╡ 040d3fff-a2cd-484c-9b5d-49d3375996fa
b = Yields.Zero(Bootstrap(),rates,mats);

# ╔═╡ b495b6ec-28c9-49d7-bf56-1b9e0e7f4213
bl = Yields.Zero(Bootstrap(Yields.LinearSpline()),rates,mats);

# ╔═╡ 501b3c90-8fa2-4b00-b04e-6d67ddddf52f
md" That's it! We've fit the rates using four different techniques. These can now be used in a variety of ways, such as calculating the `present_value`, `duration`, or `convexity` of different cashflows if you imported [ActuaryUtilities.jl](https://github.com/JuliaActuary/ActuaryUtilities.jl)"
...
"""
function p(alpha=[1,1,1,1])
	theme(:wong2)
	p = plot(legend=:topleft,xlabel="Tenor",ylabel="Continuous yield",grid=false)
	scatter!(
		mats,
		Yields.rate.(Continuous().(rates)),
		label = "Given zero rates"
	)

	curveplot!(p,b,label="Bootstrap (Spline)",alpha=alpha[1])
	curveplot!(p,bl,label="Bootstrap (Linear)",alpha=alpha[2])
	curveplot!(p,ns, label = "NelsonSiegel",alpha=alpha[3])
	curveplot!(p,nss, label = "NelsonSiegelSvensson",alpha=alpha[4])
