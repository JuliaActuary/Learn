...
# ╔═╡ c80cb8c4-ee67-11ec-0816-77676878d2af
using FinanceModels

# ╔═╡ e7f43088-a92d-43f3-8584-756cfbd9fda2
using Plots

# ╔═╡ ceb370fb-ad7f-46b7-bb10-bcb177d7f5fb
md" # Fitting Yield Curves to rates
...
# ╔═╡ 619e758c-460f-4784-ad88-9ea1518f0d4a
rates = Continuous.([0.01, 0.01, 0.03, 0.05, 0.07, 0.16, 0.35, 0.92, 1.40, 1.74, 2.31, 2.41] ./ 100)

# ╔═╡ 509cf67d-bc6f-414b-8e99-418df34d8137
mats = [1/12, 2/12, 3/12, 6/12, 1, 2, 3, 5, 7, 10, 20, 30]

# ╔═╡ f5e9023f-3f44-4668-b18a-11c3ed82169e
quotes = ZCBYield.(rates,mats)

# ╔═╡ dafaee53-cf23-489d-b179-cd000aef26d1
ns =  fit(Yield.NelsonSiegel(),quotes);

# ╔═╡ f817f52b-2a66-4667-8140-cda7a7e3ff25
nss = fit(Yield.NelsonSiegelSvensson(),quotes);

# ╔═╡ 41a6c4e9-4374-41ae-80e6-ef4a12e844aa
sw = fit(Yield.SmithWilson(ufr=0.05,α=0.1),quotes);

# ╔═╡ 0f1195c2-f565-45f1-b427-1521e8db6281
bl =  fit(Spline.Linear(), quotes, Fit.Bootstrap());

# ╔═╡ cf0c4f07-bf71-49e8-8302-4703c33c94d4
bq =  fit(Spline.Quadratic(), quotes, Fit.Bootstrap());

# ╔═╡ 9a0b2e76-50c9-4c83-82c6-ea673fd345e2
bc =  fit(Spline.Cubic(), quotes, Fit.Bootstrap());

# ╔═╡ 501b3c90-8fa2-4b00-b04e-6d67ddddf52f
md" That's it! We've fit the rates using six different techniques. These can now be used in a variety of ways, such as calculating the `present_value`, `duration`, or `convexity` of different cashflows if you imported [ActuaryUtilities.jl](https://github.com/JuliaActuary/ActuaryUtilities.jl)"

# ╔═╡ e55da4f6-1ba0-4e91-b005-22d6c5689570
md"## Visualizing the results"

# ╔═╡ e0385d98-53e1-44e7-b225-c69edc20cf9e


# ╔═╡ 1d04f511-9954-4634-9e2b-5bad6a81ac90
"""
plot the given curve onto the given plot figure
"""
...
# ╔═╡ 26187930-cade-49c3-8517-3105242d4c28
""" 
...
function p(alpha=[1,1,1,1,1,1])
	...
	curveplot!(p,sw, label = "SmithWilson",alpha=alpha[6])
 	lens!([0, 3.5], [0.0, .0045], inset = (1, bbox(.5, 0.5, 0.4, 0.4)))
	
	
end

# ╔═╡ b25427c5-fd08-4c07-ae23-e9aedb005c17
p()

# ╔═╡ 1b647af0-2d90-424a-8b24-4a9f48d511aa
anim = let
	a = [1,.25,.25,.25,.25,.25]
...
# ╔═╡ 24f70673-74fb-4818-8f7c-25b90a70e688
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "147e03d5fd870372aea98037a9a35ff4c556703f"
...
git-tree-sha1 = "c2a58d4646d16312dfb63552f383165f59fbc4b5"
...
[[deps.DataPipes]]
git-tree-sha1 = "7ee371c4d2ec6e3f735b48c5782ece9a2808c809"
...
git-tree-sha1 = "8da22299e7d4f1b5fe0c2d5b00f6fca125f46456"
uuid = "b9b1ffdd-6612-4b69-8227-7663be06e089"
version = "2.0.2"

[[deps.FinanceModels]]
deps = ["AccessibleOptimization", "Accessors", "BSplineKit", "Dates", "Distributions", "FinanceCore", "IntervalSets", "LinearAlgebra", "Optimization", "OptimizationMetaheuristics", "OptimizationOptimJL", "PrecompileTools", "Reexport", "StaticArrays", "Transducers", "UnicodePlots"]
git-tree-sha1 = "a7d1bf2dd127c9e551906c7075d36d3fd19f06da"
...
[[deps.JMcDM]]
deps = ["Requires"]
git-tree-sha1 = "bc05847a1f32adf0a403ee27addb6daae0a72e38"
