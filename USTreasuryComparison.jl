...
let 
    ...
    tenor_labels = string.(tenors)

    fig = Figure()
    ax = Axis(fig[1, 1], 
        xlabel = "maturity", 
        ylabel = "zero rate", 
        limits = (0, nothing, 0, 0.08),
        xticks = (tenors, tenor_labels),
	  	yminorticksvisible = true, yminorgridvisible = true,
        yminorticks = IntervalsBetween(4)
    )

    lines!(ax, xs, z1, label = "$d1")
    lines!(ax, xs, z2, label = "$d2")

    # Optionally: Add vertical lines for standard tenors
    vlines!(ax, tenors, color=:gray, linestyle=:dash, linewidth=1)

    dlatest = last(dates)
    if dlatest ∉ [d1, d2]
        zlatest = map(t -> rate(zero(clatest, t)), xs)
        lines!(ax, xs, zlatest, label = "$(last(dates)) (latest)",
               linestyle = :dot, linewidth = 3, alpha = 0.5)
    end
...
manifest_format = "2.0"
project_hash = "8bac58e2a31760881e98b0f618f7bd37c8a7cbe5"
